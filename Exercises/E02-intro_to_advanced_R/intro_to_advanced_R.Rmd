...
Now `df_long` corresponds to tidy format of the original dataframe.
We actually did this:

![Converting wide data to tidy format.](images/wide_to_long.png)

Why is tidy data useful for some purposes (mainly for plotting)?
...
Would it be neat to have this information alongside the tidy data (`df_long`)?
I think so:
...
**As you will see later, this tidy form of data is extremely useful for plotting.**
...
Omitting a range is also possible - in thix example you would use `-(disp:drat)`.
...
It puts `NA`s in columns not present in the second dataframe.
...
The goal of `tidyr` is to help you create [tidy data](https://tidyr.tidyverse.org/articles/tidy-data.html).
