---
title: "E05 - Multiple testing issue (Analysis of Gene Expression 2020 @ VSCHT)"
author: "Michal Kolar"
...
```{bash, eval = FALSE}
cp -r /data/shared/AGE2020/Exercises/E05-multiple_testing_issue ~/AGE2020/Exercises
```

And switch the R working directory to the current exercise: `setwd("~/AGE2020/Exercises/E04-multiple_testing_issue")`
...
plot(p[oq], xlim = c(0, 200), ylim = c(0, 0.125), col = c(rep("green", 100), rep("black", 900))[oq], pch = ".",
...
```{r}
plot(p[oq], xlim = c(0, 150), ylim = c(0, 0.05), col = c(rep("green", 100), rep("black", 900))[oq], pch = ".",
     cex = 3, ylab = "Raw p-value")
...
Create the [confusion matrix](https://en.wikipedia.org/wiki/Confusion_matrix) for different p-adjustment methods and cutoff value of 0.05, 0.1 and 0.25.
...
render("multiple_testing_issue.Rmd", output_file = "multiple_testing_issue.html")
```
