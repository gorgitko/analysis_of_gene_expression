...
Based on directory this file is located in, `here` uses `Exercises/` as the root directory.
...
The same applies for `render()` (respectively for `knitr`), but can be changed (before you call `render()`) with:
...
So this is nested dataframe for `cyl == 4`:
...
Of course you can use all of these functions for vectors outside of `mutate()`:
