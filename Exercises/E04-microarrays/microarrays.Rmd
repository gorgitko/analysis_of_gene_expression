...
```{bash, eval = FALSE}
cp -r ~/shared/AGE2021/Exercises/E04-microarrays ~/AGE2021/Exercises
```

**Assignment**

Your first assignment will be to implement the `select_var_features(m, n_top_features)` in `age_library.R`.
This functions will subset matrix `m`s rows by `n_top_features` with the highest variance across the columns (samples) and
arrange the rows of subsetted matrix by decreasing variance.

You will use this function in `plot_pca()` and `plot_heatmap()` (and similar functions if you want),
which will take `n_top_features` parameter, pass it to `select_var_features()` and do what should they do on the subsetted matrix `m`.

In this exercise, there are several cases of `n_top_features` usage in PCA and heatmaps.

The second assignment will be to implement a custom Rmd file and R script for reporting of differential expression analysis results.

Please, return the assignment in this form: 

- Make a first Rmd file named `n_top_features.Rmd` in which you will:
  - Load the Rds file with `norm_data` located in `E04-microarrays/norm_data.Rds`, and source `age_library.R`
    with implemented `select_var_features()` and other functions.
  - Make PCA plot and heatmap using `n_top_features = 1000` and colored (annotated) by `sample_group`.
- Make an R script which will loop over contrasts in the `fit` object, and a template Rmd file which will be used
  to render differential expression results from `topTable()`:
  - The `fit` object is saved in `E04-microarrays/fit.Rds`.
  - Implementation is very similar to the loop used in [*ReportingTools* section](#reportingtools),
    but instead of using `ReportingTools` functions you will use `rmarkdown::render()`.
  - Detailed instructions are given in [*Custom reporting of DEGs* section](#custom-reporting-of-degs).
  - Name the output HTML files as `<contrast_name>.html`.
- In the header of both Rmd files, put your name and e-mail to the `author` parameter.
- Copy all finished files to a directory named `E04-microarrays_assignment_<your_name>`: both Rmd files, rendered HTMLs, all R files needed to reproduce your results.
...
[2](https://commons.wikimedia.org/wiki/File:Microarray_and_sequencing_flow_cell.svg)).](`r here("E04-microarrays/images/microarray.png")`)
...
](`r here("E04-microarrays/images/ExpressionSet.png")`)
...
This can be rewritten to a matrix form as:
...
(FDR, LFC) as **differentially expressed genes** (DEGs).
...
you can see that it is possible to set p-value adjust method and also to set thresholds for
  ...
  tryCatch(
    {
      ...
      cat(glue("<a href='{REPORT_DIR}/{group}.html' target='_blank'>report</a>"))
    },
...
However, for a HTML report to be loaded correctly, you must open it from a normal filesystem, not the RStudio file browser.
So copy the `r basename(REPORT_DIR)` directory to your local computer or access the HTML files with `sshfs`.
...
- You can also use more parameters: report title (e.g. contrast name), thresholds for DEGs, sample data, ...
...
Lastly, all tables in R can be easily exported to tab-delimited formats (CSV, TSV) or Excel ([openxlsx](https://ycphs.github.io/openxlsx/index.html) package).
...
and which used the same microarray technology as in the former.
...
We will add the batch effect as an another factor:
...
The batch effect is now "eaten" by the `batches` covariate, and so we observe the correct statistics for the main biological effect of used drug (the `groups_batch` covariate).
...
save.image(here(BASE_DIR, "microarrays.RData"))
...
rmarkdown::render(here("E04-microarrays/microarrays.Rmd"), output_file = here("E04-microarrays/microarrays.html"), envir = new.env())
```
