...
You can create a symlink in your home directory:

```{bash}
ln -s /data/persistent/<user> /home/<user>/persistent
```

Files for AGE are stored in `/data/shared/AGE2020`.
Because we all have the write permission in `/data/shared` and we don't want to overwrite work of others `r emo::ji("slightly_smiling_face")`, please,
create your personal directory in which you will
copy exercise files on the beginning of each exercise:

```{bash}
mkdir -p /data/persistent/<user>/AGE2020/Exercises
```

And make a symlink in your home to this directory:

```{bash}
ln -s /data/persistent/<user>/AGE2020 ~/AGE2020
```
...
## [`sshfs`](https://github.com/libfuse/sshfs) - mount a directory on remote server

`sshfs` allows to mount directories on a remote server, through the SSH protocol.
...
For that, create a directory on your local machine, owned by you. For example:

```{bash}
mkdir ~/age_vm
```

Then you can mount a remote directory: `sshfs user@hostname:directory ~/age_vm` 

In case of VM for AGE, you can have a saved connection in `~/.ssh/config` on your local machine:
...
Then you can use, for example, `sshfs age:/home/jirinovo ~/age_vm`

***

## [`tmux`](https://github.com/tmux/tmux/wiki) - terminal multiplexer

`tmux` is a very helpful utility to manage multiple persistent shell sessions.
It is somewhat similar to quite old `screen`, but `tmux` is a better choice in my opinion.
You can install `tmux` by `sudo apt install tmux`.

`tmux` has a following hierarchy:

![In sessions, you create windows which can be split to panes.](images/tmux.png)

**The important thing is that `tmux` sessions are persistent until you restart the server.**
And so you can return back to your work after a network outage, or run a long-term task and
...
- **Shortcuts**: triggered by a prefix key followed by a command key.
  ...
  For example, to create a new window, you press `Ctrl + b` followed by `c`.
...
To start a new session: `tmux new -s my_session`

You can read a quick guide [here](https://www.hamvocke.com/blog/a-quick-and-easy-guide-to-tmux/) and
...
You may have heard about [conda](https://docs.conda.io/en/latest/), which is defaultly installed on your VM.
...
so rather install packages as the root (`sudo R`).
...
Now when you get an error, you’ll get an interactive prompt that displays the
