...
We will work at virtual machines (VM) provided by [Metacentrum](https://cloud2.metacentrum.cz/).
...
SSH config defined in `~/.ssh/config` on your local machine provides you a simple management of SSH hosts.
...
- `/data/persistent/<user>/bio-class-deb10` for your personal data.

To avoid data lose, I would recommend you to use the persistent storage.
For easier access from your home directory, create symlinks there:

```{bash}
ln -s /data/shared ~/shared
ln -s /data/persistent/$USER/bio-class-deb10 ~/persistent
```

Files for AGE are stored in `/data/shared/AGE2021`.
...
```{bash}
mkdir -p ~/persistent/AGE2021/Exercises
```

And make a symlink in your home directory:

```{bash}
ln -s ~/persistent/AGE2021 ~/AGE2021
```

Now, please, copy these files and directories to your personal directory:

```{bash}
cp -r ~/shared/AGE2021/Exercises/{_assets,E01-intro} ~/AGE2021/Exercises
```

And make a new RStudio project using the existing directory (`~/AGE2021/Exercises`):

![File > New Project...](`r here("E01-intro/images/new_project_01.png")`)

![Choose "Existing Directory"](`r here("E01-intro/images/new_project_02.png")`)

![Navigate to and select directory "~/AGE2021/Exercises"](`r here("E01-intro/images/new_project_03.png")`)
...
```{r echo = FALSE, eval = TRUE, fig.cap = "In sessions, you create windows which can be split to panes.", out.width = "100%"}
knitr::include_graphics(here("E01-intro/images/tmux.png"))
```
...
For a more comfortable experience in Linux shell, I can recommend [Fish shell](https://fishshell.com/), the *f*riendly *i*nteractive *sh*ell,
which is already installed on your VMs.
...
If you need to edit some file in Linux shell, I can recommend [micro](https://micro-editor.github.io/) editor, which is also installed on your VMs.
It's a lighweight alternative to the classic [GNU nano](https://www.nano-editor.org/), with keybindings and features very similar to editors with graphical user interface.
...
My solution is following: SSH to your VM and run `pkill -f rsession`. It will kill all running R sessions.
...
rmarkdown::render(here("E01-intro/intro.Rmd"), output_file = here("E01-intro/intro.html"))
```
