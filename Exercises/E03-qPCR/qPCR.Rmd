...
p_heatmap <- plot_heatmap(
  cp,
  sample_annotation = dplyr::select(pheno_data, `Sample Group` = sample_group, `Petri Dish` = petri_number, Confluence = confluence, Replicate = replicate),
  feature_annotation = dplyr::select(genes_df, `Gene Group` = gene_group),
  legend_title = "Cp",
...
```{r}
plot_heatmaply(gene_cor, feature_annotation = dplyr::select(genes_df, `Gene Group` = gene_group), main = main_title, legend_title = "Correlation")
```
...
plot_heatmap(delta_cp_avg_plot, sample_annotation = pheno_data_avg[, coloring_variables], title = "qPCR", legend_title = "deltaCp") %>%
...
plot_heatmap(
  lfc_wide,
  sample_annotation = dplyr::select(pheno_data_avg, `Sample Group` = sample_group),
