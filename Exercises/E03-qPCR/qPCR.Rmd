...
```{bash, eval = FALSE}
cp -r ~/shared/AGE2021/Exercises/{E03-qPCR,age_library.R} ~/AGE2021/Exercises
```
...
- `plot_hc(m, color_by)`:
  Calculate hiearchical clustering of matrix `m` columns and plot a dendrogram, whose labels are colored according to `color_by` vector.
- `plot_pca(m, sample_data, color_by)`:
  Calculate PCA of matrix `m` and make a scatterplot of first two principal compoments with points colored by `color_by` column in `sample_data`.
- `plot_heatmap(m, column_annotation, z_score = FALSE)`:
  Make a heatmap of matrix `m` with column annotation specified in `column_annotation` dataframe.
  Optionally, convert values in `m` to z-score (row-wise).
- `plot_boxplots(plot_data, x, y)`:
  Using long `plot_data`, plot boxplots of values in `y` column, splitted to groups defined in `x` column.
- `compute_m(gene, cp)`:
  Compute the M value of $C_P$ values (geNorm algorithm).
- `test_gene(gene, gene_data, gene_col, value_col, group_col)`:
  Using long `gene_data`, calculate between group statistical test (e.g. t-test) of `gene`
...
- `test_gene_table(gene_data, gene_col, value_col, group_col)`:
  The same as `test_gene()`, but calculate tests for all genes in `gene_col` column and return a dataframe of results.
...
![qPCR measurement](`r here("E03-qPCR/images/qpcr_curves.png")`)
...
We will work with data from pancreatic tumor experiment where
...
Along with measured data you usually obtain a sample sheet, but in this case we are going to parse the column names of $C_P$ matrix.
...
pheno_data <- data.frame(
  ...
  replicate = if_else(str_detect(samples, "\\(2RT\\)"), "2", "1")
) %>%
  dplyr::mutate(
...
If you are not sure what regexes above mean, go to [regex101.com](https://regex101.com/) and try them.
You can place multiple sample names on separate lines.
In regexes you just have to replace double backslashes (`\\`) by a single one. That's because backslash is used in R (and other programming languages)
for [escaping](https://en.wikipedia.org/wiki/Escape_character). Shortly, character after backslash has a special meaning (e.g. `\n` is used for newline)
and because regexes themselves use backslashes (e.g. `\d` means "any digit"), you have to tell R to treat backslash as normal character, and not as the start of escaping.

![Example from [regex101.com](https://regex101.com/)](`r here("E03-qPCR/images/regex101.png")`)
...
> Function (`compute_m()`) to be implemented within the assignment.
  Your functions should output values similar to the below ones (or at least numerically very close to them).
...
![](`r here("E03-qPCR/images/boxplot_lfc.png")`)
