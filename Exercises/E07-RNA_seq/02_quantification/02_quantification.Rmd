...
```{bash}
cp -r ~/shared/AGE2021/Exercises/E07-RNA_seq/{bin,02_quantification} ~/AGE2021/Exercises/E07-RNA_seq
cp -r ~/shared/AGE2021/Exercises/E07-RNA_seq/data/{sample_sheet_airway.csv,experiment_trimmed_filtered,genome,gsnap_index_hg37_chr20,salmon_index_hg37_chr20,rna_sequences,transcriptome} ~/AGE2021/Exercises/E07-RNA_seq/data
```
...
To make them available in the shell, you need to add this directory to the `PATH` environment variable and make these files executable:
...
# First, change to the directory to prevent a catastrophic scenario when SORTMERNA_TMP_DIR is not defined.
cd $SORTMERNA_TMP_DIR && rm -r ./* && cd -
```
...
save(list = ls(all.names = TRUE), file = here("E07-RNA_seq/02_quantification/02_quantification.RData"), envir = environment())
