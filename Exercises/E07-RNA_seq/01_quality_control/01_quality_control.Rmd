...
Finally, we use the transcript abundance for differential expression, that is, a relative change in the trascript abundance
...
On the experiment page in GEO, you can find the link to SRA on the bottom of page ("SRA Run Selector").

However, for you to know how to deal with raw data, we will try the RNA-seq workflow parts starting with raw reads
...
# Install sratools from bioconda channel.
conda install -c bioconda sra-tools
```

In `$PATH` environment variable, path to the old version of `fasterq-dump` is preceding the path to the new venv `sratools`,
and so when you run `fasterq-dump`, the old version is still executed:
...
```
/opt/bio-class/sratoolkit.2.9.6-ubuntu64/bin/fasterq-dump
```
...
```
...:/opt/bio-class/sratoolkit.2.9.6-ubuntu64/bin:...:/opt/bio-class/miniconda/envs/sratools/bin
```

Let's move on with the most trivial solution and just replace `fasterq-dump` with the full path to the new version:

```{bash}
/opt/bio-class/miniconda/envs/sratools/bin/fasterq-dump --version
```

```
"/opt/bio-class/miniconda/envs/sratools/bin/fasterq-dump" version 2.10.3
```

We are ready to use the new version of `fasterq-dump`.
...
for sample_id in ${SAMPLE_NAMES[@]}; do
  ...
  /opt/bio-class/miniconda/envs/sratools/bin/fasterq-dump \
...
Now we will look at a technical quality of out RNA-seq experiment.
...
You can inspect them in `fastqc` directory.
...
It removes adapter (technical) part of reads and filter/trim reads based on their PHRED quality score.
